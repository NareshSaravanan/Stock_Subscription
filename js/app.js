var WatchStock = React.createClass({
    ...
    watchStock: function() {
        this.props.watchStockHandler(this.state.code);
        this.setState({code: ''});
    },
    ...
    render: function () {
        return (
            <div className="row">
                <p>Available stocks for demo: MSFT, LOW, XOM, G00G, JNJ, CVS, BDX, ABT, UTX, SFM</p>
                <div className="input-group">
                    <input type="text" className="form-control" placeholder="Comma separated list of stocks to watch..." value={this.state.code} onChange={this.handleChange} />
...
var StockRow = React.createClass({
    unwatch: function() {
        this.props.unwatchStockHandler(this.props.stock.code);
    },
    render: function () {
        var lastClass = '',
...
var StockTable = React.createClass({
    render: function () {
        ...
        for (var symbol in this.props.stocks) {
            ...
            items.push(<StockRow key={stock.code} stock={stock} last={this.props.last} unwatchStockHandler={this.props.unwatchStockHandler}/>);
        }
        return (
            <div className="row">
...
var HomePage = React.createClass({
    getInitialState: function() {
        var stocks = {};
        feed.watch(['ABT', 'G00G', 'BDX', 'CVS', 'XOM', 'JNJ', 'LOW', 'MSFT', 'SFM', 'UTX']);
